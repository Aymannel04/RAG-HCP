...
Le mod\`ele BGE-M3, t\'el\'echarg\'e et mis en cache lors du premier test, se charge ici quasi instantan\'ement (pas de nouveau t\'el\'echargement). 8 documents PDF r\'eels indexes sans erreur, 455 chunks au total, filtre de langue toujours actif (4 pi\`eces jointes arabes suppl\'ementaires \'ecart\'ees). Ce lot n'a couvert que la cat\'egorie \'Economie (limite atteinte avant les listings Marché du travail/Population).

\subsection{Run complet, sans limite~: 151 documents, DOCX valid\'e}
Ayman a ensuite lanc\'e le script sans \texttt{--limite}, jusqu'\`a son terme, sur les 15 pages listing (\'Economie, March\'e du travail, Population~\&~d\'emographie). Ex\'ecution longue (gros volume, certaines publications comportant plusieurs pi\`eces jointes -- ex. le RGPH~2024 compte \`a lui seul 9 PDF distincts) mais sans aucune erreur bloquante du d\'ebut \`a la fin.

\begin{terminal}[title={PS> python -m scripts.indexer\_documents \ \ (extrait de fin de run)}]
\begin{Verbatim}
html  | ignore_html  |   0 chunks | Note sur les resultats de l'enquete nationale sur la migrati
docx  | ok           |  43 chunks | Note sur les resultats de l'enquete nationale sur la migrati
pdf   | ok           | 371 chunks | Note sur les resultats de l'enquete nationale sur la migrati
...
html  | ignore_html  |   0 chunks | Allocution de Monsieur le Haut-Commissaire au Plan a l'occas
docx  | ok           |  12 chunks | Allocution de Monsieur le Haut-Commissaire au Plan a l'occas
...
Total : 151 document(s) traite(s), 6999 chunk(s) indexe(s).
\end{Verbatim}
\end{terminal}

R\'esultat final~: \textbf{151 documents index\'es, 6999 chunks au total}. Point cl\'e~: \textbf{2 vrais fichiers \texttt{.docx}} rencontr\'es et index\'es avec succ\`es (43 et 12 chunks) -- le support DOCX (ADR 0005, section~4) est d\'esormais valid\'e en conditions r\'eelles, pas seulement par les tests avec fixture g\'en\'er\'ee. Plusieurs cas limites r\'eels ont \'et\'e correctement g\'er\'es sans interrompre le run~: liens annonc\'es comme PDF/DOCX mais dont le contenu t\'el\'echarg\'e s'av\`ere invalide (\'ecart\'es via la v\'erification de signature binaire, Sprint~1), et filtre de langue arabe actif de bout en bout, sur les pages comme sur les pi\`eces jointes individuelles.
...
Le Sprint 2 est d\'esormais valid\'e de bout en bout en conditions r\'eelles, et pas seulement fonctionnellement complet c\^ot\'e code~: chunking, embeddings BGE-M3 r\'eels, indexation Chroma/BM25, persistance SQLite, et les trois formats de fichiers cibl\'es (PDF, XLSX, DOCX) tous confirm\'es sur de vraies publications hcp.ma, \`a l'\'echelle (151 documents, 6999 chunks). Le Sprint 3 peut s'ouvrir sur des bases solides~: routage des questions, recherche structur\'ee sur les indicateurs BDS, reranking, et g\'en\'eration de r\'eponse.
